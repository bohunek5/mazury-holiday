...
for (const [lang, data] of Object.entries(translationsData)) {
    ...
    if (langProp && langProp.getKind() === SyntaxKind.PropertyAssignment) {
        ...
        if (strandaProp && strandaProp.getKind() === SyntaxKind.PropertyAssignment) {
            const strandaObj = strandaProp.getInitializerIfKindOrThrow(SyntaxKind.ObjectLiteralExpression);
