...
const getBuildingsData = () => {
    ...
    // Ensure they are sorted by ID, but C has a specific order
    buildings.A.sort((a, b) => a.id.localeCompare(b.id));
    buildings.B.sort((a, b) => a.id.localeCompare(b.id));
    buildings.C.sort((a, b) => {
        const order = ['studio', 'c', 'C301'];
                        ...
                        <div className="flex flex-col gap-3 w-full md:w-auto">
                            ...
                            <div className="flex bg-white dark:bg-slate-800 rounded-xl p-1 border border-slate-200 dark:border-slate-700 flex-wrap">
                                {['all', 'parter', 'pietro1', 'pietro2', 'pietro3'].map((floor) => (
                                    ...
                                    >
                                        {floor === 'all' ? (filtersLabels.floorAny || "Dowolna") : floor === 'parter' ? (filtersLabels.floorParter || "Parter") : floor === 'pietro1' ? (filtersLabels.floor1 || "1 Piętro") : floor === 'pietro2' ? (filtersLabels.floor2 || "2 Piętro") : (filtersLabels.floor3 || "3 Piętro")}
                                    </button>
                                ))}
                            </div>
                        </div>
                                    ...
                                    {filteredBuildings[buildingKey].map((unit) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                        ...
                                                        <div className="relative h-64 w-full overflow-hidden">
                                                            ...
                                                            <div className="absolute bottom-4 left-5 text-white pr-4">
                                                                <h3 className="text-lg md:text-xl font-bold font-sans">
                                                                    {(() => {
                                                                        if (!aptData) return unit.id;
                                                                        const features = [];
                                                                        
                                                                        const typeLow = (aptData.type || '').toLowerCase();
                                                                        const titleLow = (aptData.title || '').toLowerCase();
                                                                        const descLow = (aptData.description || '').toLowerCase();
                                                                        
                                                                        if (typeLow.includes('studio') || titleLow.includes('studio') || descLow.includes('studio')) {
                                                                            features.push(featuresLabels.studio || 'Studio');
                                                                        } 
                                                                        if (typeLow.includes('twobedroom') || titleLow.includes('dwoma sypialniami') || titleLow.includes('2 sypialni') || descLow.includes('dwoma sypialniami') || descLow.includes('2 sypialnie') || descLow.includes('2 sypialni') || descLow.includes('dwie sypialnie')) {
                                                                            features.push(featuresLabels.twoBedrooms || '2 Sypialnie');
                                                                        } 
                                                                        if (typeLow.includes('onebedroom') || titleLow.includes('jedną sypialnią') || titleLow.includes('1 sypialni') || descLow.includes('jedną sypialnią') || descLow.includes('1 sypialnią') || descLow.includes('sypialnię')) {
                                                                            features.push(featuresLabels.oneBedroom || '1 Sypialnia');
                                                                        } else if (descLow.includes('sypialni') && !features.includes('2 Sypialnie') && !features.includes('1 Sypialnia')) {
                                                                            features.push(featuresLabels.oneBedroom || '1 Sypialnia');
                                                                        }
                                                                        ...
                                                                        if (hasJacuzzi) features.push(featuresLabels.jacuzzi || 'Jacuzzi');

                                                                        const hasSauna = (aptData.amenities?.bathroom?.some((item: string) => item.toLowerCase().includes('sauna')) || false)
                                                                            || (aptData.amenities?.living?.some((item: string) => item.toLowerCase().includes('sauna')) || false)
                                                                            || titleLow.includes('sauna');
                                                                        if (hasSauna) features.push(featuresLabels.sauna || 'Sauna');

                                                                        return features.join(' | ') || aptData.title;
                                                                    })()}
                                                                </h3>
                                                            </div>
                                                        </div>
                                                ...
                                                <a
                                                    href={`https://client37851.idobooking.com/book-now/index.php?ob[${aptData?.idoBookingId || '1'}]=&showOtherOffers=true&currency=0&language=0&from_own_button=1`}
                                                    target="_blank"
