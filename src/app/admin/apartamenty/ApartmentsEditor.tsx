...
import type { CzarterData } from "@/data/czarter-data";
...
export function ApartmentsEditor({
    ...
    const [czarter, setCzarter] = useState(initialCzarter);
    ...
    return (
        <div className="space-y-12 pb-20">
            ...
            <header className="rounded-[1.5rem] md:rounded-[2.5rem] border border-white/10 bg-white/5 p-6 md:p-10 backdrop-blur-xl">
            ...
            <section className="space-y-6">
                ...
                <div className="grid gap-6">
                    {Object.values(stranda).map((apt) => (
                        <div key={apt.id} className="group rounded-[1.5rem] md:rounded-[2rem] border border-white/10 bg-white/5 p-5 md:p-8 transition-all hover:bg-white/[0.07]">
                            <div className="mb-6 flex flex-col sm:flex-row items-start justify-between gap-4">
                                <div>
                                    <h4 className="text-lg md:text-xl font-bold text-white">{apt.id} — {apt.type}</h4>
            ...
            <section className="space-y-6">
                ...
                <div className="rounded-[1.5rem] md:rounded-[2.5rem] border border-indigo-500/20 bg-indigo-500/5 p-6 md:p-10">
            ...
            <section className="space-y-6">
             ...
             <section className="space-y-6">
                ...
                <div className="rounded-[1.5rem] md:rounded-[2.5rem] border border-rose-500/20 bg-rose-500/5 p-6 md:p-10">
                    <div className="grid gap-6 md:grid-cols-3">
                        <label className="space-y-2">
                            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Cena</span>
                            <input 
             ...
             <section className="space-y-6">
                <div className="flex items-center gap-3 px-2">
                    ...
                    <h3 className="text-2xl font-sans font-bold text-white">Czarter (Stillo 30 VIP)</h3>
                </div>

                <div className="rounded-[1.5rem] md:rounded-[2.5rem] border border-cyan-500/20 bg-cyan-500/5 p-6 md:p-10">
                    <div className="grid gap-6 md:grid-cols-2">
                        <label className="space-y-2">
                            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Tytuł</span>
                            <input 
                                value={czarter.title}
                                onChange={(e) => setCzarter({...czarter, title: e.target.value})}
                                className="w-full rounded-2xl border border-white/10 bg-slate-900/50 p-4 text-white outline-none focus:border-cyan-500/50"
                            />
                        </label>
                        <label className="space-y-2">
                            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">IdoBooking ID</span>
                            <input 
                                value={czarter.idoBookingId}
                                onChange={(e) => setCzarter({...czarter, idoBookingId: e.target.value})}
                                className="w-full rounded-2xl border border-white/10 bg-slate-900/50 p-4 text-white outline-none focus:border-cyan-500/50"
                            />
                        </label>
                        <label className="space-y-2 md:col-span-2">
                            <span className="text-xs font-bold uppercase tracking-widest text-slate-500">Adres iCal URL</span>
                            <input 
                                value={czarter.icalUrl}
                                onChange={(e) => setCzarter({...czarter, icalUrl: e.target.value})}
                                className="w-full rounded-2xl border border-white/10 bg-slate-900/50 p-4 text-white outline-none focus:border-cyan-500/50"
                            />
                        </label>
                    </div>
