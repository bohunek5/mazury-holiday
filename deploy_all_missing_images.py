...
print(f"📦 Zipping targets to {ZIP_PATH}...")
...
with open(ZIP_PATH, 'rb') as f:
    for i in range(total_chunks):
        ...
        sys.stdout.write(f"\r  -> Uploading chunk {i+1}/{total_chunks}...")
        sys.stdout.flush()
        ...
        while retries > 0:
            try:
                ...
                if resp.status_code == 200 and resp.text.strip() == "ok":
                    ...
                    print(f"\n     ⚠️ Chunk {i} failed with {resp.status_code}, response: {resp.text[:50]}")
                    retries -= 1
                    time.sleep(2)
            except Exception as e:
                print(f"\n     ⚠️ Exception on chunk {i}: {e}")
                retries -= 1
                time.sleep(2)
...
print("\n🚀 Triggering partial unzip and move 14...")
resp = session.post(
    f"{WP_URL}/wp-admin/admin-post.php",
    data={
...
if resp.status_code == 200 and "ok" in resp.text:
    print("✅ Image deploy finished successfully!")
else:
