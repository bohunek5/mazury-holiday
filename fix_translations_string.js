...
let raw = fs.readFileSync('src/lib/translations.ts', 'utf8');

// We will use regex to find `"attractionsPage": { ... }` up to the next key like `"cooperationPage": {`
function replaceSection(text, langObj) {
    // The problem is the block might contain nested braces, up to infinity because of the bug.
    // Instead of regex, let's parse using a small parser.
    // Actually, we can just replace everything between `"attractionsPage": {` and `"cooperationPage": {`
    // assuming they are sequential in all languages.
    return text;
}

// But wait, what if I strip the "export type..." and then use Function('return ' + str) ?
const content = raw.split('export type Language')[0].replace('export const translations = ', '');

try {
    const fn = new Function('return ' + content);
    const obj = fn();
    
    if (obj.pl) obj.pl.attractionsPage = plAttractions;
    if (obj.en) obj.en.attractionsPage = enAttractions;
    if (obj.de) obj.de.attractionsPage = deAttractions;

    let newFile = 'export const translations = ' + JSON.stringify(obj, null, 2) + ';\n\n';
    newFile += 'export type Language = keyof typeof translations;\n';
    newFile += 'export type TranslationKey = keyof typeof translations.pl;\n';

    fs.writeFileSync('src/lib/translations.ts', newFile);
    console.log("Successfully rebuilt src/lib/translations.ts");
} catch(e) {
    console.error(e);
}
